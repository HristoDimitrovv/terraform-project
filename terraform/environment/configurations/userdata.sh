...
sed -i "s/Password1/$(aws ssm get-parameter --region eu-west-1 --name '/myapp/dbpassword' --with-decryption --output text --query 'Parameter.Value')/" web-app/web/config.php
sed -i "s|Password1|$(aws ssm get-parameter --region eu-west-1 --name "/myapp/dbpassword" --with-decryption --output text --query 'Parameter.Value')|" web-app/db/db_setup.sql
...
mysql -v --skip-ssl -u "${DB_USERNAME}" -p"$(aws ssm get-parameter --region eu-west-1 --name "/myapp/dbpassword" --with-decryption --output text --query 'Parameter.Value')" -h "${DB_HOSTNAME}" < web-app/db/db_setup.sql
